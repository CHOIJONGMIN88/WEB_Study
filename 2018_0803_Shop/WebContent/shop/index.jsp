...
<html>
 ...
 <center>
 	...
 	<!-- 관리자계정이 아닐시 로그인 버튼이 보이지 않는다. 로그인된 상태에서 관리자만 가능하도록 -->
 	
 	<c:if test="${not empty sessionScope.user and user.id eq 'admin'}">
  ...
  <!-- 로그인이 되어있으면 -->	 
 	 <c:if test="${not empty user}">
 	 	${user.name}님 환영합니다.<input type="button" value="로그아웃" onclick="location.href='../member/logout.do'">
		<input type="button" value="장바구니" onclick="location.href='cart_list.do'">
